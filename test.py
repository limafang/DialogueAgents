...
#         subprocess.run(['ffmpeg', '-f', 'concat', '-safe', '0', '-i', self.concat_file, '-c', 'copy', output_path], check=True)
#         print(f"Concatenated audio saved at {output_path}")

#     def run_batch(self, text_batches, speaker_filenames, prompts, output_filename):
#         all_filenames_batch0 = []
#         all_filenames_batch1 = []
#         refined_texts_batches = []
#         for texts, speaker_file, prompt in zip(text_batches, speaker_filenames, prompts):
...
#             refined_texts = self.process_texts(texts, params_refine_text, params_infer_code)
#             refined_texts_batches.append((refined_texts, params_refine_text, params_infer_code))


#         max_len = max(len(texts) for texts in text_batches)
#         for i in range(max_len):
#             if i < len(refined_texts_batches[0][0]):
#                 wavs = self.generate_wavs([refined_texts_batches[0][0][i]], refined_texts_batches[0][1], refined_texts_batches[0][2])
#                 filenames = self.save_wavs(wavs, f"batch0_text{i}")
#                 all_filenames_batch0.extend(filenames)
#             if i < len(refined_texts_batches[1][0]):
#                 wavs = self.generate_wavs([refined_texts_batches[1][0][i]], refined_texts_batches[1][1], refined_texts_batches[1][2])
#                 filenames = self.save_wavs(wavs, f"batch1_text{i}")
#                 all_filenames_batch1.extend(filenames)

#         self.concatenate_wavs(all_filenames_batch0, all_filenames_batch1, output_filename)
...
#             dialogues = script['conversation']
#             text_batches = [[], []]
#             speaker_filenames = ['seed_speaker_A.pt', 'seed_speaker_B.pt']  # Modify according to your speaker files
#             prompts = ['[oral_2][laugh_0][break_6]', '[oral_2][laugh_0][break_6]']

#             for dialogue in dialogues:
#                 if dialogue['speaker'] == 'A':
#                     text_batches[0].append(dialogue['text'])
#                 elif dialogue['speaker'] == 'B':
#                     text_batches[1].append(dialogue['text'])

#             output_filename = f"conversation_{script['id']}.wav"
#             self.run_batch(text_batches, speaker_filenames, prompts, output_filename)


# if __name__ == "__main__":
#     model_path = '/data/xli/speech-agent/ChatTTS/asset/speaker_emb'
#     output_dir = 'test_outputs'
#     json_path = 'scripts.json'  # Path to your JSON file

#     processor = ChatTTSProcessor(model_path, output_dir)
#     processor.process_json(json_path)
text_batches = [
    [
